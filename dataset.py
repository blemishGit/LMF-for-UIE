...
def get_patch(img_in, img_tar, patch_size, scale=1, ix=-1, iy=-1):
    ...
    # 如果图像尺寸小于所需 patch 尺寸，则自动调整 patch 大小
    if iw < ip or ih < ip:
        # 如果图像尺寸太小，动态缩小 patch 尺寸
        ip = min(iw, ih)
        tp = ip * scale
        print(f"Image too small, reducing patch size to ({ip}, {tp})")
...
# def get_patch(img_in, img_tar, patch_size, scale=1, ix=-1, iy=-1):
#     (ih, iw) = img_in.size  # 获取输入图像的高和宽
#
#     patch_mult = scale
#     tp = patch_mult * patch_size  # target patch size
#     ip = tp // scale  # input patch size
#
#     # 如果图像尺寸小于所需 patch 尺寸，则自动调整 patch 大小
#     if iw < ip or ih < ip:
#         # 如果图像尺寸太小，动态缩小 patch 尺寸
...
#     (tx, ty) = (scale * ix, scale * iy)
#
#     # 确保 crop 的坐标顺序正确 (left, top, right, bottom)
#     img_in = img_in.crop((ix, iy, ix + ip, iy + ip))
#     img_tar = img_tar.crop((tx, ty, tx + tp, ty + tp))
#
#     # 打印调试信息，查看实际提取的 patch 尺寸
#     # print(f"Patch info - ix: {ix}, iy: {iy}, ip: {ip}, tx: {tx}, ty: {ty}, tp: {tp}")
#
#     info_patch = {
#         'ix': ix, 'iy': iy, 'ip': ip, 'tx': tx, 'ty': ty, 'tp': tp}
#
